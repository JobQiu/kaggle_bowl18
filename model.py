...
class MaskRCNN():
    ...
    def unmold_detections(self, detections, mrcnn_mask, image_shape, window, probability_mask=False):
        ...
        for i in range(N):
            ...
            if probability_mask:
                probabily_mask_ = utils.unmold_mask(masks[i], boxes[i], image_shape, binary=False)
                probabily_masks_.append(probabily_mask_)
            full_masks.append(full_mask)
        full_masks = np.stack(full_masks, axis=-1) \
            if full_masks else np.empty((0,) + masks.shape[1:3])

        return boxes, class_ids, scores, full_masks, probabily_masks_

    def detect(self, images, verbose=0, probablymask=False, separate_nuclei=False):
        ...
        for i, image in enumerate(images):
            final_rois, final_class_ids, final_scores, final_masks, probably_masks = \
                self.unmold_detections(detections[i], mrcnn_mask[i],
                                       image.shape, windows[i],
                                       probability_mask=probablymask)
            mask_count = final_scores.shape[0]
            if separate_nuclei:
                overlapping_mask_pair = []
                for i in range(mask_count):
                    for j in range(i + 1, mask_count):
                        if (np.any(np.logical_and(final_masks[:, :, i], final_masks[:, :, j]))):
                            overlapping_mask_pair.append((i, j))
                if len(overlapping_mask_pair) > 0:
                    for (i,j) in overlapping_mask_pair:
                        print(i,j)
                    pass
